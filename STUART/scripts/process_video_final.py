...
def post_processing(keypoint_trajectories,
    ...
    for kp_name, positions in keypoint_trajectories.items():
        ...
        trajectory_data.append({
            ...
            'time' : keypoint_area_data.get(kp_name, {}).get('time_inside', 0),
        })
    ...
    # Guardar distancias recorridas dentro del área central
    with open(os.path.join(trajectories_dir, 'distancias_recorridas_area_central.txt'), 'w') as f:
        for kp_name, data in keypoint_area_data.items():
            distancia = data['distance_inside']
            f.write(f"Keypoint {kp_name}: {distancia:.4f} metros\n")

    # Guardar tiempos y entradas al área central
    with open(os.path.join(trajectories_dir, 'tiempo_area_central.txt'), 'w') as f:
        for kp_name, data in keypoint_area_data.items():
            tiempo = data['time_inside']
            enter_count = data['enter_count']
            exit_count = data['exit_count']
            f.write(f"Keypoint {kp_name}:\n")
            f.write(f"  Tiempo dentro del area central: {tiempo:.4f} segundos\n")
            f.write(f"  Veces que entro: {enter_count}\n")
            f.write(f"  Veces que salio: {exit_count}\n\n")
